...
//        mLayoutParagraph = mSaveLayoutPara ;
    ...
    if(same == false || mLayoutRest == true)
    {
        ...
        QString str = strformat("Formatting %d %% ...", perc).c_str() ;
...
{
    QString t1 = strformat("Command %s not implemented (yet)", t.toUtf8().constData()).c_str() ;
...
void cEditorCtrl::LayoutFullDocument(bool force)
{
    ...
    printf("Performing Detailed Timers\n") ;
...
//            bool wholeword = findandreplace.mWholeWords->isChecked() ;
            ...
            else if(entire)
            {
                ...
                QString t = strformat("Replaced %ld items", count).c_str() ;
...
void cEditorCtrl::FindAgain(void)
{
    if(mSearchBackwards == false)
    {
        ...
        if(fpos == mDocument.GetTextSize())
        {
            QString temp = strformat("Search word: %s  not found.", mSearchText.c_str()).c_str() ;
            QMessageBox msgBox(QMessageBox::Information, "Not found", temp, QMessageBox::Ok, this) ;
            msgBox.exec() ;
        }
        else
...
void cEditorCtrl::PageLayout(void) 
{
    ...
    for(int loop = 1; loop < 200; loop++)
    {
        ...
        if(mMeasure == MSR_CENTIMETERS || mMeasure == MSR_MILLIMETERS)
        {
            ...
            full = strformat("%s, %d x %d mm", name.toUtf8().constData(), static_cast<int>(size.width()), static_cast<int>(size.height())).c_str() ;
        }
        else
        {
            size = psize.size(QPageSize::Inch) ;
            full = strformat("%s, %.2f x %.2f in", name.toUtf8().constData(), size.width(), size.height()).c_str() ;
        }
...
// @TODO page orientation set
    ...
    {
        case MSR_INCHES :
            text = strformat("%0.2f\"", value / TWIPSPERINCH).c_str() ;
            break ;
        
        case MSR_MILLIMETERS :
            text = strformat("%0.2f\"", value / TWIPSPERMM).c_str() ;
            break ;

        default :
            text = strformat("%0.2f\"", value / TWIPSPERCM).c_str() ;
            break ;
    }
    ...
    if(ecode != 0)
    {
        ...
        if(newtype != mPaperId)
        {
            ...
            QString t = strformat(".pt %d (%s)", newpaper, pagelayout.mPaperSize->currentText().toUtf8().constData()).c_str() ;
        ...
        if(FuzzyCompare(value, mLayout->mPageOffsetOdd)) //  value != mLayout->mPageOffsetOdd)
        {
            ...
            {
                case MSR_INCHES :
                    t = strformat(".poo %0.2f\"", value / TWIPSPERINCH).c_str() ;
                    break ;
                
                case MSR_MILLIMETERS :
                    t = strformat(".poo %0.2fmm", value / TWIPSPERMM).c_str() ;
                    break ;

                default :
                    t = strformat(".poo %0.2fcm", value / TWIPSPERCM).c_str() ;
                    break ;
            }
            mDocument.Insert(t.toUtf8().constData()) ;
            mDocument.Insert(HARD_RETURN) ;
        ...
        if(value != mLayout->mPageOffsetEven)
        {
            ...
            {
                case MSR_INCHES :
                    t = strformat(".poe %0.2f\"", value / TWIPSPERINCH).c_str() ;
                    break ;
                
                case MSR_MILLIMETERS :
                    t = strformat(".poe %0.2fmm", value / TWIPSPERMM).c_str() ;
                    break ;

                default :
                    t = strformat(".poe %0.2fcm", value / TWIPSPERCM).c_str() ;
                    break ;
            }
            mDocument.Insert(t.toUtf8().constData()) ;
            mDocument.Insert(HARD_RETURN) ;
        ...
        if(value != mLayout->mTopMargin)
        {
            ...
            {
                case MSR_INCHES :
                    t = strformat(".mt %0.2f\"", value / TWIPSPERINCH).c_str() ;
                    break ;
                
                case MSR_MILLIMETERS :
                    t = strformat(".mt %0.2fmm", value / TWIPSPERMM).c_str() ;
                    break ;

                default :
                    t = strformat(".mt %0.2fcm", value / TWIPSPERCM).c_str() ;
                    break ;
            }
            mDocument.Insert(t.toUtf8().constData()) ;
            mDocument.Insert(HARD_RETURN) ;
        ...
        if(value != mLayout->mBottomMargin)
        {
            ...
            {
                case MSR_INCHES :
                    t = strformat(".mb %0.2f\"", value / TWIPSPERINCH).c_str() ;
                    break ;
                
                case MSR_MILLIMETERS :
                    t = strformat(".mb %0.2fmm", value / TWIPSPERMM).c_str() ;
                    break ;

                default :
                    t = strformat(".mb %0.2fcm", value / TWIPSPERCM).c_str() ;
                    break ;
            }
            mDocument.Insert(t.toUtf8().constData()) ;
            mDocument.Insert(HARD_RETURN) ;
        ...
        if(value != mLayout->mRightMargin)
        {
            ...
            {
                case MSR_INCHES :
                    t = strformat(".rm %0.2f\"", value / TWIPSPERINCH).c_str() ;
                    break ;
                
                case MSR_MILLIMETERS :
                    t = strformat(".rm %0.2fmm", value / TWIPSPERMM).c_str() ;
                    break ;

                default :
                    t = strformat(".rm %0.2fcm", value / TWIPSPERCM).c_str() ;
                    break ;
            }
            mDocument.Insert(t.toUtf8().constData()) ;
            mDocument.Insert(HARD_RETURN) ;
        ...
        if(value != mLayout->mHeaderMargin)
        {
            ...
            {
                case MSR_INCHES :
                    t = strformat(".hm %0.2f\"", value / TWIPSPERINCH).c_str() ;
                    break ;
                
                case MSR_MILLIMETERS :
                    t = strformat(".hm %0.2fmm", value / TWIPSPERMM).c_str() ;
                    break ;

                default :
                    t = strformat(".hm %0.2fcm", value / TWIPSPERCM).c_str() ;
                    break ;
            }
            mDocument.Insert(t.toUtf8().constData()) ;
            mDocument.Insert(HARD_RETURN) ;
        ...
        if(value != mLayout->mFooterMargin)
        {
            ...
            {
                case MSR_INCHES :
                    t = strformat(".fm %0.2f\"", value / TWIPSPERINCH).c_str() ;
                    break ;
                
                case MSR_MILLIMETERS :
                    t = strformat(".fm %0.2fmm", value / TWIPSPERMM).c_str() ;
                    break ;

                default :
                    t = strformat(".fm %0.2fcm", value / TWIPSPERCM).c_str() ;
                    break ;
            }
            mDocument.Insert(t.toUtf8().constData()) ;
            mDocument.Insert(HARD_RETURN) ;
...
void cEditorCtrl::Preferences(int tab)
{
    ...
    switch(tab)
    {
        ...
        case 2:
            ui.mTabWidget->setCurrentWidget(ui.tab_2) ;
            break ;

        case 3:
            ui.mTabWidget->setCurrentWidget(ui.tab_3) ;
            break ;
    }
...
//                        newdisplay.controls.push_back(rect);
            ...
            if (paraposition /*- caratadjust */ == caratposition)
            {
                ...
                QFontInfo f(font) ;
                mStatusFont = strformat("%s %.1f", f.family().toUtf8().constData(), f.pointSizeF() / mFontScale).c_str() ;
