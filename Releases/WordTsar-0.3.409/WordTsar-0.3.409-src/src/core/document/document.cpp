...
//    size_t len = GraphemeCount(paraiter->buffer, offsets);
    ...
    SaveOffsets(currentparagraphnumber, pos, offsets);
...
bool cDocument::Delete(POSITION_T position, POSITION_T length)
{
    ...
    for (POSITION_T ctr = position; ctr < position + length; ctr++)
    {
        ...
        // refresh our grapheme offsets
        SaveOffsets(currentparagraphnumber, position, offsets);
    }
...
//    if(mRecalcTextSize == true)
    {
        ...
        auto iter = mParagraphData.begin() + size ;

        // we cast here, because when we insert, we make sure our buffer never goes over POSITION codepoints
//        vector<POSITION_T> offsets ;
//        size_t len = iter->offsets.size(); // GraphemeCount(iter->buffer, offsets);
//        mTextSize = iter->index + static_cast<POSITION_T>(len) ;
//        mRecalcTextSize = false ;
...
/*
    ...
    {
        mUndo.pop_front() ;
        mUndoCarat.pop_front() ;
    }

    mUndo.push_back(mParagraphData) ;
    mUndoCarat.push_back(mCurrentPosition) ;
...
size_t cDocument::GraphemeCount(string &text, vector<POSITION_T> &offsets)
{
    ...
    do
    {
        size = utf8proc_iterate(reinterpret_cast<const utf8proc_uint8_t*> (&text[index]), static_cast<long>(text.length() - index), &codepoint);
...
/// @param  show   [IN] - if true, obeys mShowControl state
///
...
///
/////////////////////////////////////////////////////////////////////////////
string cDocument::GetParagraphText(PARAGRAPH_T para, bool show)
{
    string text ;
...
// this needs to be grapheme aware
        ...
        size_t start1 = static_cast<size_t>(start) ;            // conversion here just to get rid of warnings
...
//        paraiter->buffer = paraiter->buffer.take(static_cast<size_t>(offset)) ;
        ...
        if(position != mParagraphData[static_cast<size_t>(paragraph)].index)
        {
            vector<POSITION_T> offsets;
            SaveOffsets(currentparagraphnumber, position, offsets);
            SaveOffsets(currentparagraphnumber + 1, position, offsets);
...
void cDocument::DecrementAttributes(POSITION_T position, POSITION_T length, bool changeparaindex)
{
    ...
    MY_ASSERT(position >= 0)
    MY_ASSERT(position <= GetTextSize())
// GBR    MY_ASSERT(position + length <= bsize)
...
/////////////////////////////////////////////////////////////////////////////
///
/// @param  paragraph   [IN] - the nth grapheme to find
/// @param  cp          [IN] - the codepoint array for this paragraph
///
/// @return size_t - the offset in the array for grapheme at position
///
/// @brief Copies a UTF8 encoded paragraph buffer into a vector of char32_t codepoints
///
/// This is needed to convert between UTF8-CPP and CPP_UNICODELIB UTF8 libraries
/////////////////////////////////////////////////////////////////////////////
void cDocument::ConvertCodepointstoParagraph(PARAGRAPH_T paragraph, vector<utf8proc_int32_t> &cp)
{
/*
    immer::flex_vector<char> newbuffer ;
    auto transient = newbuffer.transient() ;
    unsigned char str[5] ;
    memset(str, 0, 5) ;

    for(size_t loop = 0; loop < cp.size(); loop++)
    {
        utf8proc_encode_char(cp[loop], &str[0]) ;
    }
    ...
    for(auto loop = 0; loop < 5; loop++)
    {
        ...
        transient.push_back(static_cast<char>(str[loop])) ;
    }

    mParagraphData[static_cast<size_t>(paragraph)].buffer = transient.persistent() ;
*/
}
...
///
/// @param  paragraph   [IN] - the nth grapheme to find
/// @param position     [IN] - the position to start looking for new graphemes
...
/// This is needed to convert between UTF8-CPP and CPP_UNICODELIB UTF8 libraries
/////////////////////////////////////////////////////////////////////////////
void cDocument::SaveOffsets(PARAGRAPH_T paragraph, POSITION_T position, vector<POSITION_T>& offsets)
{
    size_t newlen = 0;

    /// @todo we actually do the entire paragraph. No optimizations yet.
    newlen = GraphemeCount(mParagraphData[paragraph].buffer, offsets);
