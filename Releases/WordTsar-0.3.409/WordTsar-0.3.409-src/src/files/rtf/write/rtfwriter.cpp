...
//        if(text.at(pos) < STYLE_END_OF_STYLES)
        if(text.at(pos) == MARKER_CHAR)
        {
            QChar ch = mDocument->GetControlChar(mCurrentPosition) ;
            CreateModifiers(ch) ;
//            CreateModifiers(text.at(pos)) ;
            ControlSpace() ;
        }
        else if(text.at(pos) > 255) ///< @todo This is wrong (RTF 1.9.1 spec page 14)
        {
        ...
        else
        {
            ...
            if(text.at(pos) < 32 || text.at(pos) > 126)
            {
                cCodePage437 cp ;
...
{
    QString out ;
    out.sprintf("\\%s", control.toUtf8().constData()) ;
...
{
    QString out ;
    out.sprintf("\\%s%d", control.toUtf8().constData(), parameter) ;
    mFile.write(out.toUtf8().constData()) ;
    mNewLine = false ;
}
...
{
    QString out ;
    out.sprintf("\\%s %s", control.toUtf8().constData(), text.toUtf8().constData()) ;
    mFile.write(out.toUtf8().constData()) ;
    mNewLine = false ;
}
